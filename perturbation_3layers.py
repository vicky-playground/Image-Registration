...
for trial in range(num_trials):
    ...
    # Create the model
    net = ThreeLayerNet(input_size, hidden_size, num_classes).to(device)
    print(net)
    
    # Initialize input weights for perturbation
    input_weights = copy.deepcopy(net.state_dict())
    ...
    # Calculate the distance between initial weights and final weights
    weight_distance = 0.0
    for param_name in input_weights:
        weight_distance += torch.norm(input_weights[param_name] - net.state_dict()[param_name])

    # Calculate upper and lower bounds for perturbation
    upper_bound = perturbation_scale * weight_distance
    lower_bound = -upper_bound

    # Create perturbation in weight space
    perturbation = {}
    for param_name in input_weights:
        perturbation[param_name] = torch.empty_like(input_weights[param_name]).uniform_(lower_bound, upper_bound)

    # Apply perturbation to initial weights
    perturbed_weights = {}
    for param_name in input_weights:
        perturbed_weights[param_name] = input_weights[param_name] + perturbation[param_name]

    # Load perturbed weights to the model for the next trial
    net.load_state_dict(perturbed_weights)
